...
const logConfigSchema = z.object({
  enable: z.boolean(),
  path: z.string().default('logs/raybot.log'),
  rotationCount: z.number().min(1).default(10),
...
const form = useForm({
  ...
  initialValues: props.logConfig.file,
})
...
<template>
  <form class="flex flex-col w-full max-w-lg space-y-6" @submit="onSubmit">
    ...
    <FormField v-slot="{ componentField }" name="path">
      <FormItem>
        <FormLabel>Log File Name</FormLabel>
        <Input v-bind="componentField" />
        <FormMessage />
      </FormItem>
    </FormField>

    <FormField v-slot="{ componentField }" name="rotationCount">
      <FormItem>
        <FormLabel>Max Log Files Number</FormLabel>
