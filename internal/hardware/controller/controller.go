...
func (c *controller) trackingPICCommandACK(ctx context.Context, id string) error {
	...
	log.InfoContext(ctx, "start tracking PIC command ack")

	doneCh := make(chan struct{})
	c.subscriber.Subscribe(ctx, events.PICCmdAckTopic, func(msg *eventbus.Message) {
		ev, ok := msg.Payload.(events.PICCmdAckEvent)
		if !ok {
			log.ErrorContext(ctx, "invalid event", slog.Any("event", msg.Payload))
			return
		}

		if ev.ID == id {
			if ev.Success {
				log.InfoContext(ctx, "PIC command ack success")
			} else {
				log.ErrorContext(ctx, "PIC command ack failed")
			}
			close(doneCh)
		}
	})
	...
	select {
	case <-doneCh:
		log.InfoContext(ctx, "stop tracking PIC command ack")
		return nil

	case <-time.After(c.cfg.PIC.CommandACKTimeout):
		log.ErrorContext(ctx, "PIC command ack timeout")
...
func (c *controller) trackingESPCommandACK(ctx context.Context, id string) error {
	log := c.log.With(slog.String("id", id))
	log.InfoContext(ctx, "start tracking ESP command ack")
	...
	c.subscriber.Subscribe(ctx, events.ESPCmdAckTopic, func(msg *eventbus.Message) {
		...
		if ev.ID == id {
			if ev.Success {
				log.InfoContext(ctx, "ESP command ack success")
			} else {
				log.ErrorContext(ctx, "ESP command ack failed")
			}
			close(doneCh)
		}
	})
	...
	select {
	case <-doneCh:
		log.InfoContext(ctx, "stop tracking ESP command ack")
		return nil

	case <-time.After(c.cfg.ESP.CommandACKTimeout):
		log.ErrorContext(ctx, "ESP command ack timeout")
		return ErrCommandACKTimeout
