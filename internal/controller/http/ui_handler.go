...
func (s HTTPService) RegisterUIHandler(r chi.Router) {
	// Get the UI dist subdirectory from the embedded filesystem
	uiFS, err := fs.Sub(raybot.UIFS, "ui/dist")
	if err != nil {
		s.log.Error("Failed to get UI filesystem", slog.Any("error", err))
		return
	}
