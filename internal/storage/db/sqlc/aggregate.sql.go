...
type RobotStateGetRow struct {
	Battery          interface{} `json:"battery"`
	BatteryCharge    interface{} `json:"battery_charge"`
	BatteryDischarge interface{} `json:"battery_discharge"`
	DistanceSensor   interface{} `json:"distance_sensor"`
	DriveMotor       interface{} `json:"drive_motor"`
	LiftMotor        interface{} `json:"lift_motor"`
	Location         interface{} `json:"location"`
	Cargo            interface{} `json:"cargo"`
	CargoDoorMotor   interface{} `json:"cargo_door_motor"`
}

func (q *Queries) RobotStateGet(ctx context.Context, db DBTX) (RobotStateGetRow, error) {
	row := db.QueryRowContext(ctx, robotStateGet)
	var i RobotStateGetRow
	err := row.Scan(
		&i.Battery,
		&i.BatteryCharge,
		&i.BatteryDischarge,
