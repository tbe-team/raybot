...
func (e cargoOpenExecutor) trackingCargoDoorUntilOpen(ctx context.Context) {
	...
	defer func() {
		e.log.InfoContext(ctx, "stop tracking cargo door open")
		cancel()
	}()

	doneCh := make(chan struct{})
	e.log.InfoContext(ctx, "start tracking cargo door open")
	e.subscriber.Subscribe(ctx, events.CargoDoorUpdatedTopic, func(msg *eventbus.Message) {
		...
		if ev.IsOpen {
			e.log.InfoContext(ctx, "cargo door open completed")
			close(doneCh)
		}
	})
